...
async function convertFile() {
  ...
  const libraryName = 'cfd';
  const libPrefix = 'cfd';
  const structPrefix = 'jsonapi';
  const libNamespace = 'cfd::api';
  const errorClassName = 'CfdError';
  const hasStructErrorOutput = false;
  ...
  const outJsonSourceFolderPath = `${__dirname}/../../${libraryName}/src/jsonapi/autogen/`;
  const outJsonHeaderFolderPath = `${__dirname}/../../${libraryName}/src/jsonapi/autogen/`;
