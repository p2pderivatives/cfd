  ...
  if (ret == kCfdSuccess) {
    ...
    if (ret == kCfdSuccess) {
      ...
      ret = CfdAddConfidentialTxDerSign(
        handle, kTxData, "57a15002d066ce52573d674df925c9bc0f1164849420705f2cfad8a68111230f", 0,
        true, signature, kCfdSigHashAll, false, &tx_string);
      EXPECT_EQ(kCfdSuccess, ret);
    ...
    if (ret == kCfdSuccess) {
      ret = CfdAddConfidentialTxSign(
        ...
        true, "03f942716865bb9b62678d99aa34de4632249d066d99de2b5a2e542e54908450d6", &tx_string2);
      EXPECT_EQ(kCfdSuccess, ret);
    }
    if (ret == kCfdSuccess) {
  ...
  if (ret == kCfdSuccess) {
    ...
    if (ret == kCfdSuccess) {
      ret = CfdFinalizeElementsMultisigSign(
          ...
          "57a15002d066ce52573d674df925c9bc0f1164849420705f2cfad8a68111230f", 0,
          kCfdP2sh, nullptr, multisig_script, &tx_string);
      EXPECT_EQ(kCfdSuccess, ret);
      if (ret == kCfdSuccess) {
